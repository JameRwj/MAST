    ...
    USE BasicData,ONLY : Limiter,Riemann_Solver,Mainf,epsilon,&
        id,jd,Initialization_Method,TestCase,Reconstruction_Method

    IMPLICIT NONE

    CHARACTER*50 :: CharTemp1,CharTemp2
    CHARACTER*100:: CharGrid,CharLimiter,CharSolver,&
        CharInitialization,CharTestCase,CharReconstruction
    INTEGER :: key,error
    ...
    IF (error==1) THEN
        WRITE(*,*)&
        "There is at least one error in Setting.dat. Please check."
        PAUSE
        STOP
    END IF
