    ...
    ALLOCATE(u(1-ghostLayers:id+ghostLayers-1,1-ghostLayers:jd+ghostLayers-1),&
             v(1-ghostLayers:id+ghostLayers-1,1-ghostLayers:jd+ghostLayers-1),&
			 rho(1-ghostLayers:id+ghostLayers-1,1-ghostLayers:jd+ghostLayers-1),&
             p(1-ghostLayers:id+ghostLayers-1,1-ghostLayers:jd+ghostLayers-1))
    ALLOCATE(x(1:id,1:jd),y(1:id,1:jd))  
    ALLOCATE(sxx(1:id,1:jd-1),sxy(1:id,1:jd-1),&
             syx(1:id-1,1:jd),syy(1:id-1,1:jd),vol(1:id-1,1:jd-1))
    ALLOCATE(nxx(1:id,1:jd-1),nxy(1:id,1:jd-1),&
             nyx(1:id-1,1:jd),nyy(1:id-1,1:jd))
